export default {
  async fetch(request, env) {
    ...
    // 1. GENERATE RSS XML FOR KEYWORDS / STOCKS / WEBSITES
    if (url.pathname === "/generate") {
      const target = url.searchParams.get("target");
      if (!target) return new Response("Missing target query", { status: 400 });
...
<rss version="2.0">
  <channel>
    <title>Feed for ${target}</title>
    <link>https://${url.hostname}/generate?target=${encodeURIComponent(target)}</link>
    <description>Generated news feed for ${target}</description>
    ${xmlItems}
  </channel>
</rss>`;
    ...
    // 2. PARSE RSS/ATOM FEEDS TO JSON
    if (url.pathname === "/rss") {
