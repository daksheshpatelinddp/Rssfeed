...
function parseRSS(xml){
  ...
  for(const block of itemMatches){
    ...
    if(title||link)items.push({id:guid,title,description,url:link,date:date||new Date().toISOString()});
  }
  return items;
}
...
function parseAtom(xml){
  ...
  for(const block of entryMatches){
    ...
    if(title||link)items.push({id,title,description,url:link,date:date||new Date().toISOString()});
  }
  return items;
}
...
async function fetchUpstream(url){
  ...
  for(let attempt=0;attempt<3;attempt++){
    try{
      ...
      lastError=error?.name==="AbortError"?"Source request timed out":error?.message||"Source request failed";
    }
...
export default{
  async fetch(request,env){
    ...
    if(requestUrl.pathname==="/news"){
      ...
      if(query.length>100)return json({ok:false,error:"Keyword search is limited to 100 characters"},400);
      try{
        const data=await fetchNewsData(query,env);
        const items=normalizeNewsData(data);
        return json({ok:true,source:"NewsData.io",count:items.length,items});
      }catch(error){
        return json({ok:false,error:error?.message||"NewsData request failed"},502);
      }
    }

    if(requestUrl.pathname!=="/rss")return json({ok:false,error:"Endpoint not found"},404);

    const source=requestUrl.searchParams.get("url");
    if(!source)return json({ok:false,error:"Missing url parameter"},400);

    let feedUrl;
    try{feedUrl=new URL(source)}catch{return json({ok:false,error:"Invalid RSS URL"},400)}
    if(!["http:","https:"].includes(feedUrl.protocol))return json({ok:false,error:"Only HTTP and HTTPS URLs are allowed"},400);

    try{
      const response=await fetchUpstream(feedUrl.toString());
      if(!response.ok)return json({ok:false,error:`RSS source returned HTTP ${response.status}`,source:feedUrl.hostname},502);
      const xml=await response.text();
      if(!xml||xml.length<20)return json({ok:false,error:"RSS source returned an empty response"},502);
      const items=parseFeed(xml);
      if(!items.length)return json({ok:false,error:"The URL did not contain a readable RSS or Atom feed",source:feedUrl.hostname},422);
      return json({ok:true,source:feedUrl.toString(),count:items.length,items:items.slice(0,100)});
    }catch(error){
